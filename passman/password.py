...
class WindowNewPassword(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.geometry(
            "+%d+%d" %
            (self.parent.winfo_x() // 4 + 320,
             self.parent.winfo_y() // 4 + 400)
        )
        ...
        self.button_show_new_password = ButtonGenerator(
            master=self,
            text="Show",
            command=lambda: helper_functions.
            toggle_password(self.new_password_field,
                            self.button_show_new_password),
            x=5 + 24 * 9,
            y=5 + 20 * 2 + 20 * 2 + 20,
            width=50,
            height=24
    ...
    def __save_new_password(self):
        ...
        new_password_data = [(
            password_address_value,
            password_name_value,
            password_field_value,
            password_description_value
        )]
        ...
        self.parent.password_list.insert(
            tk.END,
            password_name_value + " " + password_address_value
        )
...
class WindowDeletePassword():
    ...
    def __init__(self, parent):
        ...
        self.window_delete_password = ConfirmationGenerator("password deletion", )

        self.label_delete_password = LabelGenerator(
            master=self.window_delete_password,
            font=( "Times New Roman", 12 ),
            text=
            f"Are you sure you want to delete password ''?",  # add password description!
            x=380 / 2 - 370 / 2,
            y=10,
        )
    ...
    def confirm_password_deletion(self):
        ...
        cursor.execute(
            f"DELETE FROM passman WHERE password_name = '{password_values_list[0]}' AND password_address = '{password_values_list[1]}';"
        )

        connection.commit()

        listbox_data = cursor.execute(
            "SELECT password_name, password_address FROM passman;"
        )
        ...
        self.notification_window = NotificationGenerator(
            text="Password successfully deleted!"
        )
